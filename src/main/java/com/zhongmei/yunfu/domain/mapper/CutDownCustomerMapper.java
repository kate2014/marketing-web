...
public interface CutDownCustomerMapper extends BaseMapper<CutDownCustomerEntity> {
    ...
    @Select("select m.`name` ,m.`enabled_flag`, m.`begin_time` ,m.`end_time` ,m.start_price,m.`end_price` ,m.`validity_period`, m.`img_url` ,c.state, c.id,c.cut_down_id,c.`current_price` ,c.`join_count` \n" +
