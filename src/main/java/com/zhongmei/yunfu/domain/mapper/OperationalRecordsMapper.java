...
public interface OperationalRecordsMapper extends BaseMapper<OperationalRecordsEntity> {
    ...
    @Select("SELECT a.customer_id as customerId,a.customer_name as customerName, b.customer_phone as customerPhone,b.wx_open_id as wxOpenId,b.wx_photo as wxPhoto,b.wx_name as wxName, a.count as operationalCount,a.server_create_time as serverCreateTime FROM \n" +
            "(SELECT count(w.id) as count,w.`customer_id` as customer_id ,w.`customer_name`,w.`server_create_time`  FROM `wx_trade_customer` w \n" +
            "where w.brand_identy = ${brandIdenty} and w.shop_identy = ${shopIdenty} and w.marketing_id = ${activityId} and w.enabled_flag = 1 and w.status_flag = 1 and w.type = 4 " +
    ...
    List<OperationalRecordsEntity> querySalesEffect(@Param("ew") Condition wrapper,@Param("brandIdenty") Long brandIdenty,@Param("shopIdenty") Long shopIdenty,@Param("activityId") Long activityId);
}
